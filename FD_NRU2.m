function [SE,I]=FD_NRU2(user_num,n_BS,RF,beta)    
    %% parameter setup===========================================================
    %clc;
    %clear;
...
%     hold off;
    ...
    H_SI=sqrt(10^(-beta/10))*rice_matrix(Kdb,n_BS,m_BS);
    ...
    pl_u=10*log10((c/(4*pi*fc))^(2)./(dis_u.*dis_u))-20;
    pl_d=10*log10((c/(4*pi*fc))^(2)./(dis_d.*dis_d))-20;
    ...
    for k=1:dl_user
        for s=1:ul_user
                pl1(k,s)=10*log10((c/(4*pi*fc))^(2)/((u1_x(s)-u2_x(k))^(2)+(u1_y(s)-u2_y(k))^(2)))-20;
    ...
    for k=1:ul_user
        pl3(k)=10*log10((c/(4*pi*fc))^(2)/((x_wifi-u1_x(k))^(2)+(y_wifi-u1_y(k))^(2)))-20;
        H3(k)=sqrt(10^(pl3(k)/10))*mmWave_matrix(m_user,1);
    end

    %wifi transmitter to cellular
    H4=zeros(n_BS,1);H5=zeros(1,dl_user);pl5=zeros(1,dl_user);
    pl4=10*log10((c/(4*pi*fc))^(2)/(Wifi_tx'*Wifi_tx))-20;
    H4=sqrt(10^(pl4/10))*mmWave_matrix(n_BS,1);
    for k=1:dl_user
        pl5(k)=10*log10((c/(4*pi*fc))^(2)/((x_wifi-8-u2_x(k))^(2)+(y_wifi-u2_y(k))^(2)))-20;
    ...
    %% PDD algorithm=============================================================
    %initialization
    p_wifi=0.1;p_user=1;p_BS=10; %max tx power
    I_W2B=p_wifi*(H4*H4'); 
    I_W2U=p_wifi*(H5.*H5);
    I=1e-12;%interference to wifi
    N=1e-12;
    [SE1,R_u1,R_d1,I1] = HD_ZFBF(ul_user,dl_user,H_u,H_d,H2,H3,N,p_user,p_BS,n_BS,I_W2B,I_W2U);
    [SE2,R_u2,R_d2,I2] = PDD_original1(ul_user,dl_user,H_SI,H_u,H_d,H1,H2,H3,N,I,p_user,p_BS,m_BS,n_BS,RF,n_user,I_W2B,I_W2U);
    [SE3,R_u3,R_d3,I3] = PDD_original(ul_user,dl_user,H_SI,H_u,H_d,H1,H2,H3,N,I,p_user,p_BS,m_BS,n_BS,RF,n_user,I_W2B,I_W2U);
