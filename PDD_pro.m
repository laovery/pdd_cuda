function [sum_SE,R_u,R_d,I1,I_th] = PDD_pro(ul_user,dl_user,H_SI,H_u,H_d,H1,H2,H3,N,I,p_user,p_BS,m_BS,n_BS,RF,n_user,I_W2B,I_W2U)
    %I_th=I/(1+ul_user);
    ...
    while (cv>=c1)
        ...
        while(delta>=c2) 
            ...
            [V,p_u,I_th]=primal_decomposition(X,a2,Y,b2,H2,H3,p_BS,p_user,I,ul_user);
       ...
       %========calclate sum rate=========
        ...
        rec_u=zeros(n_BS);
        for n=1:ul_user
            rec_u=rec_u+H_SI*V_RF*V_BB(:,n)*V_BB(:,n)'*V_RF'*H_SI'+p_u(n)*H_u(:,n)*H_u(:,n)';
        end
