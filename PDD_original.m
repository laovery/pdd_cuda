function [sum_SE,SE,r_u,r_d,I_SI,I_user] = PDD_original(ul_user,dl_user,H_SI,H_u,H_d,H1,H2,H3,N,I,p_user,p_BS,m_BS,n_BS,RF,n_user,I_W2B,I_W2U,alphe,temp)
    ...
    I_SI = real(trace(H_SI*V_RF*(V_BB*V_BB')*V_RF'*H_SI'));
    I_SI = 10*log10(I_SI / 10^(-3));
    I_u = zeros(1,dl_user);
    for k = 1: dl_user
        for n = 1:ul_user
            I_u(k) = I_u(k) + p_u(n)*(H1(k,n)*H1(k,n)');
        end
    end
    I_user = sum(I_u);
    I_user = 10*log10(I_user / 10^(-3));
